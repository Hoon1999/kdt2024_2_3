...
public class OrderCompleteDetailDialogController {
	...
	public void makeOrderDetailDialog(int orderNum) {
		...
		try {
			Parent parent = FXMLLoader.load(getClass().getResource("OrderCompleteDetailDialog.fxml"));
			...
			Button btnOrderIncomplete = (Button) parent.lookup("#btnOrderIncomplete");
			VBox vbOrderList = (VBox) parent.lookup("#vbOrderList");
			ObservableList orderDetailList = vbOrderList.getChildren();
			
			
			// 주문 상세 내역. (주문번호로 가져옴).
			Order detailOrder = AdminMainPageController.getMainPageController().getOrderManager().findCompletedOrderByOrderNum(orderNum);
			// 주문 번호 레이블.
			...
			btnOrderIncomplete.setOnAction(new EventHandler<ActionEvent>() {
				@Override
				public void handle(ActionEvent arg0) {
					// 주문 완료를 다시 접수로 변경..
					System.out.println("주문 완료 취소 (접수로 변경)");
					
					// 주문 완료 취소(접수 변경) 처리.
					AdminMainPageController.getMainPageController().getOrderManager().incompleteOrder(detailOrder);
