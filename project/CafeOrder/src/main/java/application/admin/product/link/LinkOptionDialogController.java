...
public class LinkOptionDialogController {
	
	// 다이얼로그 실행.
	public void makeLinkOptionDialog(int productId) {
		...
		try {
			Parent parent = FXMLLoader.load(getClass().getResource("LinkOptionDialog.fxml"));
			...
			Button btnLinkOption = (Button) parent.lookup("#btnLinkOption");
			TableView<?> tvUnlinkedOptionList = (TableView<?>) parent.lookup("#tvUnlinkedOptionList");
			
			// 테이블 뷰 컬럼 초기화.
			tvUnlinkedOptionList.getColumns().setAll(new LinkOptionDisplayTableData().getColumns());
			
			lbProductName.setText(selectedProduct.getName());
			
			// 현재 선택된 상품에 연결되지 않은 모든 옵션 리스트.
			List tableDataList = new LinkedList<OptionTableData>();
			for (int optionId : selectedProduct.getUnlinkedOptionList()) {
			...
			tvUnlinkedOptionList.getItems().setAll(tableDataList);
			...
			// 옵션 연결 버튼.
			btnLinkOption.setOnAction(new EventHandler<ActionEvent>() {
				@Override
				public void handle(ActionEvent arg0) {
					...
					// 테이블의 모든 행을 돌면서,
					for (Object object : tvUnlinkedOptionList.getItems()) {
						// 체크박스 선택된 행만 모아서,
					...
					// 상품에 연결된 옵션 추가 처리.
					selectedProduct.addLinkedOptions(selectedIdList);
			...
			System.out.println("!LinkOptionDialogController : " + e.getMessage());
			e.printStackTrace();
			System.out.println("~!LinkOptionDialogController : " + e.getMessage());
		}
